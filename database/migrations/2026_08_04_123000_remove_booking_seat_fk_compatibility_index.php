...
return new class extends Migration
{
    ...
    private const PERMANENT_INDEX = 'booking_seats_showtime_seat_index';
    ...
    public function up(): void
    {
        ...
        if ($this->upDecision($inventory) === 'DROP') {
    ...
    private function upDecision(array $inventory): string
    {
        ...
        if (! $inventory['table'] || ! $inventory['showtime_column'] || ! $this->hasShowtimeForeignKey($inventory['foreign_keys'])) {
            throw new RuntimeException(
                'Cannot remove '.self::COMPATIBILITY_INDEX
                .' because the booking_seats table, showtime_id column, or its foreign key has unexpected metadata. '
                .'No DDL was executed.',
            );
        }

        return $this->hasAlternativeSupport($inventory['indexes']) ? 'DROP' : 'NOOP';
    }
    ...
    public function down(): void
    {
        ...
        if ($compatibility === [] && $this->needsCompatibilityForJulyRollback($inventory['indexes'])) {
            DB::statement(
    ...
    private function hasAlternativeSupport(array $indexes): bool
    {
        foreach ($indexes as $name => $rows) {
            if ($name !== self::COMPATIBILITY_INDEX && $this->beginsWithShowtime($rows)) {
                return true;
            }
        }
    ...
    /** @param array<string, list<array<string, mixed>>> $indexes */
    private function needsCompatibilityForJulyRollback(array $indexes): bool
    {
        if (($indexes[self::OLD_UNIQUE_INDEX] ?? []) !== []) {
            return false;
        }

        $permanent = $indexes[self::PERMANENT_INDEX] ?? [];

        return count($permanent) === 2
            && (int) ($permanent[0]['NON_UNIQUE'] ?? 0) === 1
            && $this->beginsWithShowtime($permanent)
            && (int) ($permanent[1]['NON_UNIQUE'] ?? 0) === 1
            && (int) ($permanent[1]['SEQ_IN_INDEX'] ?? 0) === 2
            && ($permanent[1]['COLUMN_NAME'] ?? null) === 'seat_id'
            && ($permanent[1]['SUB_PART'] ?? null) === null;
    }
