...
<div class="space-y-6">
    ...
    @else
        <div class="space-y-3">@foreach($bookings as $booking)
            @php($printable = (bool) $booking->can_print)
            <article class="cinema-card grid gap-4 p-5 lg:grid-cols-[1fr_1fr_auto] lg:items-center">
                <div>
                    <a class="font-mono text-lg font-black text-brand-start" href="{{ route('staff.tickets.operations',$booking) }}">{{ $booking->booking_code }}</a>
                    <p class="mt-1 text-sm app-muted">{{ $booking->created_at->format('H:i') }} · {{ $booking->sales_channel === 'counter' ? 'Tại quầy' : 'Online' }} · {{ $booking->status_label }}</p>
                    <a class="mt-2 inline-block text-sm font-bold text-brand-start" href="{{ route('staff.tickets.operations',$booking) }}">{{ $printable ? 'Mở vận hành & in' : 'Xem trạng thái đơn' }} →</a>
                </div>
                <div><p class="font-bold app-text">{{ $booking->showtime?->movie?->title }}</p><p class="text-sm app-muted">{{ $booking->showtime_label }} · {{ $booking->showtime?->room?->name }} · {{ $booking->seat_codes }}</p></div>
                <div class="lg:text-right"><p class="font-black app-text">{{ $booking->formatted_total }}</p><p class="text-xs app-muted">Tạo: {{ $booking->createdByStaff?->name ?? 'Khách online' }} · Thu: {{ $booking->authoritativePayment?->settledBy?->name ?? '—' }}</p><p class="mt-1 text-xs font-bold {{ $printable ? 'text-success' : 'text-warning' }}">In: {{ $printable ? ($booking->ticketPrint?->status_label ?? 'Chưa in') : 'Không đủ điều kiện in' }} @if($printable && ($booking->ticketPrint?->attempts_count ?? 0) > 1)· In lại {{ $booking->ticketPrint->attempts_count - 1 }} lần @endif</p></div>
            </article>
        @endforeach</div>
