...
<div class="space-y-6">
    ...
    <form method="GET" class="cinema-card grid gap-4 p-5 sm:grid-cols-3">
        ...
        <label class="cinema-label">Trạng thái<select class="cinema-input mt-1" name="status"><option value="">Tất cả</option>@foreach(['pending_payment'=>'Đang giữ','paid'=>'Đã thanh toán','used'=>'Đã soát','cancelled'=>'Đã hủy','expired'=>'Hết hạn'] as $value=>$label)<option value="{{ $value }}" @selected(request('status')===$value)>{{ $label }}</option>@endforeach</select></label>
