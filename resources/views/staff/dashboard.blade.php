            ...
            @else
                <div class="divide-y app-border">
                    @foreach($showtimes as $showtime)
                        ...
                        @endphp
                        <article class="grid gap-3 p-5 md:grid-cols-[6rem_1fr_auto] md:items-center"><div><p class="text-2xl font-black app-heading">{{ $startsAt->format('H:i') }}</p><p class="text-xs font-bold text-brand-start">{{ $status }}</p></div><div><h3 class="font-extrabold app-text">{{ $showtime->movie->title }}</h3><p class="mt-1 text-sm app-muted">{{ $showtime->room->room_type ?: '2D' }} · {{ $showtime->room->name }} · Còn {{ $remaining }}/{{ (int)$showtime->operational_seats_count }} ghế</p></div>@if($startsAt->isFuture())<a class="btn-secondary" href="{{ route('staff.counter.seats',$showtime) }}">Chọn ghế</a>@endif</article>
                    @endforeach
                </div>
            @endif
        </section>
