...
<div class="mx-auto max-w-5xl space-y-6">
    <header><p class="text-sm font-bold text-brand-start">{{ $booking->booking_code }}</p><h1 class="mt-2 text-3xl font-extrabold app-heading">{{ $cashPayment ? 'Bán vé tại quầy thành công' : 'Kiểm tra trước khi thu tiền' }}</h1></header>
    ...
    @if($cashPayment)
        <section class="cinema-card border border-success/40 p-6"><p class="font-extrabold text-success">Đã xác nhận thu tiền mặt</p><p class="mt-2 app-muted">Người thu: {{ $cashPayment->settledBy?->name ?? '—' }} · {{ $cashPayment->settled_at?->format('d/m/Y H:i:s') }} · {{ $cashPayment->transaction_code }}</p><div class="mt-5 flex flex-wrap gap-3"><a class="btn-primary" href="{{ route('staff.tickets.operations',$booking) }}">In vé cứng</a><a class="btn-secondary" href="{{ route('staff.tickets.check') }}">Soát vé sau</a><a class="btn-secondary" href="{{ route('staff.counter.review',$booking) }}">Xem đơn</a></div></section>
    @else
        <div class="flex flex-wrap justify-end gap-3">@can('counter_sales.cancel')<form method="POST" action="{{ route('staff.counter.cancel',$booking) }}">@csrf<button class="btn-secondary" type="submit">Hủy đơn và giải phóng ghế</button></form>@endcan @can('counter_sales.settle')<form method="POST" action="{{ route('staff.counter.cash',$booking) }}">@csrf<button class="btn-primary" type="submit">Xác nhận đã thu tiền mặt</button></form>@endcan</div>
    @endif
</div>
@endsection
