...
<form method="POST" action="{{ $editing ? route('admin.cinemas.update', $cinema) : route('admin.cinemas.store') }}" class="admin-form-card max-w-4xl">
    ...
    <div class="grid gap-5 md:grid-cols-2">
        @foreach([['code','Mã chi nhánh','CG'],['name','Tên chi nhánh','MovieMate Cầu Giấy'],['address','Địa chỉ','Số nhà, đường, quận/huyện'],['city','Tỉnh / thành phố','Hà Nội'],['district','Quận / huyện','Cầu Giấy'],['country','Quốc gia','Việt Nam'],['phone','Điện thoại','']] as [$field,$label,$placeholder])
            <div class="{{ $field === 'address' ? 'md:col-span-2' : '' }}"><label class="admin-label" for="{{ $field }}">{{ $label }}</label><input id="{{ $field }}" name="{{ $field }}" value="{{ old($field, $cinema->{$field}) }}" class="admin-input" placeholder="{{ $placeholder }}" {{ in_array($field, ['code','name','address','city']) ? 'required' : '' }} @if($field === 'code') maxlength="32" pattern="[A-Za-z0-9-]+" data-validation-url="{{ route('admin.validation.field') }}" data-validation-rule="cinema.code" data-validation-record="{{ $cinema->exists ? $cinema->getKey() : '' }}" @endif>@error($field)<p class="mt-1 text-sm text-error">{{ $message }}</p>@enderror</div>
        @endforeach
        <div><label class="admin-label" for="latitude">Vĩ độ</label><input id="latitude" name="latitude" type="number" step="0.0000001" min="-90" max="90" value="{{ old('latitude', $cinema->latitude) }}" class="admin-input"><p class="mt-1 text-xs app-muted">Dùng để gợi ý chi nhánh gần khách hàng.</p>@error('latitude')<p class="mt-1 text-sm text-error">{{ $message }}</p>@enderror</div>
        <div><label class="admin-label" for="longitude">Kinh độ</label><input id="longitude" name="longitude" type="number" step="0.0000001" min="-180" max="180" value="{{ old('longitude', $cinema->longitude) }}" class="admin-input"><p class="mt-1 text-xs app-muted">Dùng để gợi ý chi nhánh gần khách hàng.</p>@error('longitude')<p class="mt-1 text-sm text-error">{{ $message }}</p>@enderror</div>
