...
<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        @can('seats.manage') @if($room->status === 'active')<a href="{{ route('admin.rooms.layout.show', $room) }}" class="btn-primary">Mở trình thiết kế</a>@endif @endcan
    </div>
    <div class="cinema-card overflow-x-auto p-6">
        ...
        <div class="mx-auto grid w-max gap-1.5" style="grid-template-columns: repeat({{ $layout->columns }}, 2.35rem)">
            ...
            @for($y=1; $y <= $layout->rows; $y++) @for($x=1; $x <= $layout->columns; $x++)
                ...
                @else<span class="flex h-9 w-9 items-center justify-center rounded-lg border text-[9px] font-bold {{ $seat->status !== 'active' ? 'bg-gray-300 text-gray-600 border-gray-400' : ($seat->type === 'vip' ? 'bg-ai-start/10 text-ai-start border-ai-start/50' : ($seat->type === 'couple' ? 'bg-warning/10 text-warning border-warning/50' : 'app-input app-muted')) }}" aria-label="Ghế {{ $seat->seat_code }}, {{ $seat->type_label }}, {{ $seat->status_label }}">{{ $seat->seat_code }}</span>
                @endif
            @endfor @endfor
        </div>
