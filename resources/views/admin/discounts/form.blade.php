...
@if($used)<div class="mb-5 rounded-xl border border-warning/30 bg-warning/10 p-4 text-sm text-warning" role="status"><strong>Đã phát sinh sử dụng — nội dung khuyến mãi đã được khóa.</strong><span class="mt-1 block">Mọi usage, kể cả đã released, đều giữ nguyên khóa định nghĩa kinh doanh và phạm vi chi nhánh; chỉ trạng thái vòng đời có thể thay đổi.</span></div>@endif
<div class="mb-5 rounded-xl border app-border p-4 text-sm app-muted"><p>Mỗi đơn đặt vé áp dụng tối đa một mã khuyến mãi.</p><p class="mt-1">Đơn tối thiểu được tính trên tổng tiền vé + đồ ăn trước khuyến mãi.</p><p class="mt-1">Thời gian hiệu lực được kiểm tra theo giờ địa phương của chi nhánh nơi đặt vé; tại đúng thời điểm kết thúc, mã không còn hiệu lực.</p></div>
<div class="grid gap-5 md:grid-cols-2">
<div><label class="admin-label" for="promotion-code">Mã</label><input id="promotion-code" name="code" value="{{ old('code', $discount->code) }}" class="admin-input uppercase" required @disabled($used)></div>
...
<div data-fixed-field><label class="admin-label" for="discount-amount">Số tiền giảm (VNĐ)</label><input id="discount-amount" type="number" min="1" data-promotion-name="discount_amount_vnd" @if($selectedType === 'fixed') name="discount_amount_vnd" @endif value="{{ old('discount_amount_vnd',$discount->discount_amount_vnd) }}" class="admin-input" @disabled($used)></div>
<div data-percentage-field><label class="admin-label" for="discount-percent">Tỷ lệ giảm (%)</label><input id="discount-percent" type="number" min="1" max="100" data-promotion-name="discount_percent" @if($selectedType === 'percentage') name="discount_percent" @endif value="{{ old('discount_percent',$discount->discount_percent) }}" class="admin-input" @disabled($used)></div>
<div data-percentage-field><label class="admin-label" for="maximum-discount">Mức giảm tối đa (VNĐ)</label><input id="maximum-discount" type="number" min="1" data-promotion-name="maximum_discount_vnd" @if($selectedType === 'percentage') name="maximum_discount_vnd" @endif value="{{ old('maximum_discount_vnd',$discount->maximum_discount_vnd) }}" class="admin-input" aria-describedby="maximum-discount-help" @disabled($used)><p id="maximum-discount-help" class="mt-1 text-xs app-muted">Để trống nếu không đặt giới hạn riêng. Giá trị 0 không có nghĩa là không giới hạn.</p></div>
