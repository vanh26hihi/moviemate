...
<div class="mt-6 grid gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(300px,1fr)]">
    <section class="app-card rounded-2xl border app-border" aria-labelledby="booking-history-title">
        <div class="border-b app-border p-5 sm:p-6">
            <div class="flex flex-wrap items-end justify-between gap-3"><div><h2 id="booking-history-title" class="text-xl font-black app-heading">Lịch sử đặt vé</h2><p class="mt-1 text-sm app-muted">{{ $bookings->total() }} đơn thuộc phạm vi chi nhánh hiện tại.</p></div>@if(array_filter($filters))<a href="{{ route('admin.users.show', $managedUser) }}" class="text-sm font-bold text-brand-start hover:underline">Xóa bộ lọc</a>@endif</div>
