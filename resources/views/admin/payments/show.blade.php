...
<div class="space-y-6">
    <header class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
            ...
            <p class="mt-2 app-muted">{{ strtoupper($payment->provider) }} · {{ $payment->status_label }} · {{ $payment->status === \App\Models\Payment::STATUS_REVIEW ? 'Cần xem xét' : 'Không ở trạng thái review' }} @if($isAuthoritative)· Giao dịch có thẩm quyền @endif</p>
        </div>
    ...
    <div class="grid gap-6 xl:grid-cols-2">
        <section class="cinema-card p-6"><h2 class="text-xl font-extrabold app-heading">Thông tin giao dịch</h2><dl class="mt-4 grid gap-4 sm:grid-cols-2">
            <div><dt class="text-sm app-muted">Provider</dt><dd class="font-bold app-text">{{ strtoupper($payment->provider) }}</dd></div>
            ...
            <div><dt class="text-sm app-muted">Lần truy vấn gần nhất</dt><dd class="font-bold app-text">{{ $payment->last_queried_at?->format('d/m/Y H:i:s') ?? 'Chưa truy vấn' }}</dd></div>
        ...
        <section class="cinema-card p-6"><h2 class="text-xl font-extrabold app-heading">Đơn đặt vé</h2><dl class="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <div><dt class="text-sm app-muted">Khách hàng</dt><dd class="font-bold app-text">{{ $payment->booking?->user?->name ?? 'Khách đặt vé' }} · {{ \App\Support\PrivacyMask::email($payment->booking?->user?->email) }}</dd></div>
    ...
    <section class="cinema-card overflow-hidden"><div class="border-b app-border p-6"><h2 class="text-xl font-extrabold app-heading">Các lần thử liên quan</h2></div><div class="overflow-x-auto"><table class="admin-table"><thead><tr><th>ID</th><th>Provider</th><th class="text-right">Số tiền</th><th>Trạng thái</th><th>Vai trò</th><th>Xác minh</th><th>Ngày tạo</th></tr></thead><tbody>@foreach($attempts as $attempt)<tr class="{{ $attempt->id === $payment->id ? 'bg-brand-start/5' : '' }}"><td><a class="font-bold text-brand-start" href="{{ route('admin.payments.show', $attempt) }}">#{{ $attempt->id }}</a></td><td>{{ strtoupper($attempt->provider) }}</td><td class="text-right">{{ number_format($attempt->amount, 0, ',', '.') }} {{ $attempt->currency }}</td><td>{{ $attempt->status_label }}</td><td>{{ $payment->booking?->authoritativePayment?->id === $attempt->id ? 'Có thẩm quyền' : 'Lần thử' }}</td><td>{{ $attempt->verified_at?->format('d/m/Y H:i') ?? 'Chưa' }}</td><td>{{ $attempt->created_at?->format('d/m/Y H:i') }}</td></tr>@endforeach</tbody></table></div></section>

    <div class="grid gap-6 xl:grid-cols-2">
        <section class="cinema-card overflow-hidden"><div class="border-b app-border p-6"><h2 class="text-xl font-extrabold app-heading">Lịch sử đối soát review</h2></div><div class="overflow-x-auto"><table class="admin-table"><thead><tr><th>Thời gian</th><th>Người thực hiện</th><th>Kết quả</th><th>Trạng thái</th></tr></thead><tbody>@forelse($reviewEvents as $event)<tr><td>{{ $event->created_at?->format('d/m/Y H:i:s') }}</td><td>{{ $event->actor?->name ?? '#'.$event->actor_user_id }}</td><td>{{ $event->provider_result_category }}</td><td>{{ $event->previous_status }} → {{ $event->resulting_status }}</td></tr>@empty<tr><td colspan="4" class="py-8 text-center app-muted">Chưa có sự kiện đối soát review.</td></tr>@endforelse</tbody></table></div></section>
