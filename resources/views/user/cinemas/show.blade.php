    ...
    <div class="grid gap-8 lg:grid-cols-[260px_1fr]">
        ...
        <div>
            <form method="GET" action="{{ route('cinemas.show', $cinema->code) }}" data-showtime-filter-form data-filter-endpoint="{{ route('showtimes.filter') }}" data-filter-context="cinema" data-cinema-code="{{ $cinema->code }}" class="cinema-card mb-6 p-4">
                <fieldset><legend class="font-bold app-text">Ngày xem lịch</legend><div class="mt-3 flex gap-2 overflow-x-auto pb-2">@foreach($dates as $date)<button id="cinema-date-{{ $date['date'] }}" type="submit" name="date" value="{{ $date['date'] }}" aria-pressed="{{ $selectedDate === $date['date'] ? 'true' : 'false' }}" class="shrink-0 rounded-2xl border px-4 py-3 text-sm font-bold {{ $selectedDate === $date['date'] ? 'border-brand-start bg-brand-start text-white' : 'app-border app-secondary app-text' }}"><span class="block">{{ $date['label'] }}</span><span class="block">{{ $date['day'] }}</span></button>@endforeach</div></fieldset>
            </form>
