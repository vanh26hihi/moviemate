    ...
    <div class="relative w-full max-w-md cinema-card rounded-3xl p-7 sm:p-9">
        ...
        <div class="space-y-5" aria-disabled="true">
            <div>
                ...
                <input id="email" name="email" type="email" value="{{ old('email') }}" autocomplete="email" class="app-input w-full px-4 py-3 border app-border rounded-xl focus:outline-none focus:border-brand-start" placeholder="ban@example.com" disabled>
                @error('email')<p class="mt-2 text-xs font-semibold text-error">{{ $message }}</p>@enderror
            </div>
            <button type="button" disabled class="w-full py-3 rounded-xl bg-gradient-to-r from-brand-start to-brand-end text-white font-bold disabled:cursor-not-allowed disabled:opacity-50">Gửi hướng dẫn</button>
        </div>

        <p class="mt-4 rounded-xl border app-border app-secondary px-4 py-3 text-xs app-muted">Tính năng đặt lại mật khẩu đang được hoàn thiện.</p>
        @if (Route::has('login'))
