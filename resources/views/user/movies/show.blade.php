...
<div class="cinema-surface relative overflow-hidden">
    ...
    <div class="relative mx-auto max-w-7xl px-4 py-10 sm:px-6 md:py-14 lg:px-8">
        ...
        <section id="reviews" class="mt-14" aria-labelledby="reviews-title">
            <div class="flex items-end justify-between gap-4"><div><h2 id="reviews-title" class="text-2xl font-extrabold app-text md:text-3xl">Đánh giá từ khách hàng</h2><p class="mt-1 text-sm app-muted">Tài khoản sở hữu đơn đã thanh toán có thể đánh giá sau khi suất chiếu kết thúc.</p></div><span class="text-sm font-bold text-brand-start">{{ $movie->reviews_count }} đánh giá</span></div>
            @auth
                @if($reviewBooking)
                ...
                @else<p class="cinema-card mt-6 rounded-2xl p-4 app-muted">Bạn có thể đánh giá sau khi thanh toán và suất chiếu kết thúc.</p>@endif
