@php
    $releaseDate = $movie->release_date
        ? \Carbon\Carbon::parse($movie->release_date)->format('d/m/Y')
        : 'Đang cập nhật';

    $isNowShowing = $movie->status === 'now_showing';

    $genresText = $movie->genres
        ? $movie->genres->pluck('name')->take(2)->join(', ')
        : '';

    $genresText = $genresText ?: 'Đang cập nhật';

    $ratingValue = (float) ($movie->reviews_avg_rating ?? 0);
    $ratingText = number_format($ratingValue, 1);

    $reviewsCount = (int) ($movie->reviews_count ?? 0);

    /*
    |--------------------------------------------------------------------------
    | Badge HOT
    |--------------------------------------------------------------------------
    | Phim có điểm từ 8 trở lên hoặc có từ 10 đánh giá trở lên.
    */
    $isHot = $ratingValue >= 8 || $reviewsCount >= 10;

    /*
    |--------------------------------------------------------------------------
    | Badge MỚI
    |--------------------------------------------------------------------------
    | Phim có ngày khởi chiếu trong vòng 30 ngày gần đây.
    */
    $isNew = false;

    if ($movie->release_date) {
        $releaseCarbon = \Carbon\Carbon::parse($movie->release_date);

        $isNew = $releaseCarbon->between(
            now()->subDays(30)->startOfDay(),
            now()->addDays(1)->endOfDay()
        );
    }
@endphp

<article
    data-movie-card="{{ $movie->id }}"
    class="movie-card group dark-surface border border-white/10 rounded-[1.25rem]
    overflow-hidden transition-all duration-300
    hover:-translate-y-1 hover:border-brand-start/60
    hover:shadow-xl hover:shadow-black/20"
>
    <a
        href="{{ route('user.movies.show', $movie->slug) }}"
        class="block"
    >
        <div data-movie-poster class="poster-frame relative overflow-hidden transition-[filter] duration-500 group-hover:brightness-105">
            @if($movie->poster_url)
                <img
                    src="{{ $movie->poster_url }}"
                    alt="{{ $movie->title }}"
                    loading="lazy"
                    class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                >
            @else
                <div class="fallback-poster">
                    <i class="ph-fill ph-film-slate"></i>
                    <strong class="text-lg">MovieMate</strong>
                    <span>{{ $movie->title ?? 'Phim MovieMate' }}</span>
                </div>
            @endif

            {{-- Badge trạng thái và độ tuổi --}}
            <div class="absolute inset-x-0 top-0 z-10 p-3 flex items-start justify-between gap-2">
                <div class="flex flex-col items-start gap-2">
                    <span
                        class="inline-flex items-center rounded-full px-2.5 py-1
                        text-[11px] font-bold text-white
                        {{ $isNowShowing ? 'bg-brand-start' : 'bg-ai-start' }}"
                    >
                        {{ $isNowShowing ? 'Đang chiếu' : 'Sắp chiếu' }}
                    </span>

                    @if($isHot)
                        <span class="inline-flex items-center gap-1 rounded-full bg-orange-500 px-2.5 py-1 text-[11px] font-extrabold text-white">
                            <i class="ph-fill ph-fire"></i>
                            HOT
                        </span>
                    @elseif($isNew)
                        <span class="inline-flex items-center rounded-full bg-emerald-500 px-2.5 py-1 text-[11px] font-extrabold text-white">
                            MỚI
                        </span>
                    @endif
                </div>

                @if($movie->age_rating)
                    <span class="rounded-full bg-black/55 px-2.5 py-1 text-[11px] font-bold text-white backdrop-blur">
                        {{ $movie->age_rating }}
                    </span>
                @endif
            </div>
        </div>
    </a>

    <div class="p-4">
        <h3 class="text-white font-bold text-sm sm:text-base line-clamp-2 min-h-[2.75rem]">
            <a
                href="{{ route('user.movies.show', $movie->slug) }}"
                class="hover:text-brand-start transition-colors"
            >
                {{ $movie->title ?? 'Chưa có tên phim' }}
            </a>
        </h3>

        <p class="mt-2 text-xs text-slate-300 line-clamp-1">
            {{ $genresText }}
        </p>

        {{-- Điểm và số đánh giá --}}
        <div class="mt-3 flex items-center justify-between gap-2 text-xs">
            <span class="inline-flex items-center gap-1 font-bold text-yellow-400">
                <i class="ph-fill ph-star"></i>
                {{ $ratingText }}
            </span>

            <span class="text-slate-400">
                {{ $reviewsCount }} đánh giá
            </span>
        </div>

        <div class="mt-3 grid grid-cols-2 gap-2 text-slate-300 text-xs">
            <span class="inline-flex items-center gap-1">
                <i class="ph ph-clock"></i>

                {{ $movie->duration
                    ? $movie->duration . ' phút'
                    : 'Đang cập nhật' }}
            </span>

            <span class="inline-flex items-center gap-1 justify-end">
                <i class="ph ph-calendar-blank"></i>
                {{ $releaseDate }}
            </span>
        </div>

        <div class="mt-4 grid grid-cols-2 gap-2">
            <a
                href="{{ route('user.movies.show', $movie->slug) }}"
                class="inline-flex items-center justify-center gap-2
                rounded-xl border border-white/10 px-3 py-2
                text-xs font-bold text-white transition-colors
                hover:bg-white/10 hover:border-white/20"
            >
                <i class="ph ph-info"></i>
                Chi tiết
            </a>

            <a
                data-movie-booking-action
                href="{{ route('user.movies.show', $movie->slug) }}#showtimes"
                class="inline-flex items-center justify-center gap-2
                rounded-xl primary-gradient px-3 py-2
                text-xs font-bold text-white transition-all
                hover:shadow-lg hover:shadow-brand-start/25"
            >
                <i class="ph-fill ph-ticket"></i>
                Đặt vé
            </a>
        </div>
    </div>
</article>
